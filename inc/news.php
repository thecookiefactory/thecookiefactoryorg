...
if (isset($_GET["id"]) && is_numeric($_GET["id"])) { // DISPLAY ONE PIECE OF NEWS
	...
	if (mysql_num_rows($query) == 1) {
		...
		echo "<div class='article-header'><span class='article-title'>".$row["title"]."</span><span class='article-metadata'>";	
		echo "by ".$row["author"]." &ndash; on ".$row["date"]."</span></div><br></span>";
		echo "<div class='article-body'><span class='article-text'><p>".nl2br($row["text"])."</p></span></div>";
		if ($row["comments"] == 1) { // COMMENTS
			if (isset($_POST["cp"])) {
				$newsid = $_GET["id"];
			...
			if (checkuser())
				echo "<form action='?p=news&id=".$_GET["id"]."' method='post'><textarea name='text'></textarea><input type='submit' name='cp' /></form>";
			else
				echo "you have to be logged in to post comments";
		} else 
			echo "commenting disabled";
	} else
		echo "No newspost found matching this id!";

} else { // DISPLAY ALL THE NEWS

	$query = mysql_query("SELECT * FROM news ORDER BY id DESC");

	while ($row = mysql_fetch_assoc($query)) { // TITLE, AUTHOR & DATE
		echo "<div class='article-header'><span class='article-title'><a href='?p=news&amp;id=".$row["id"]."'>".$row["title"]."</a></span><span class='article-metadata'>";
		if ($row["comments"] == 1) {
		...
		echo "by ".$row["author"]." &ndash; on ".$row["date"]."</span></div><br />";		
		echo "<div class='article-body'><span class='article-text'><p>".nl2br($row["text"])."</p></span></div><hr class='article-separator' />";
	}
}
