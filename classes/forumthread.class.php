...
class forumthread extends master {
    ...
    public function __construct($id = null) {
        ...
        if ($id != null) {
            ...
            try {
                ...
                $squery = $con->prepare('
                    SELECT *, BIN(`forumthreads`.`closed`)
                    FROM `forumthreads`
    ...
    public function returnArray($loc = null) {
        ...
        if ($loc == 'main') {
            ...
            try {
                ...
                $selectPosts = $con->prepare('
                    SELECT `forumposts`.`id`
                    FROM `forumposts`
                    WHERE `forumposts`.`threadid` = :id
                ');
    ...
    public function commentProcess() {
        ...
        if (isset($_POST['cp']) && isset($_POST['text']) && validField($_POST['text']) && $user->isReal() && !$this->isClosed()) {
            ...
            if (strlen($text) > 20000) {
                ...
                try {
                    ...
                    $insertComment = $con->prepare('
                        INSERT INTO `forumposts`
                        VALUES(DEFAULT, :text, :author, now(), DEFAULT, :id)
                    ');
                    ...
                    $updateLastDate = $con->prepare('
                        UPDATE `forumthreads`
                        SET `forumthreads`.`lastdate` = now()
                        WHERE `forumthreads`.`id` = :id
                    ');
    ...
    public function addNew() {
        ...
        if (isset($_POST['addnew']) && (isset($_POST['cat']) && validField($_POST['cat'])) && (isset($_POST['title']) && validField($_POST['title'])) && (isset($_POST['text']) && validField($_POST['text']))) {
            ...
            if (strlen($title) > 37) {
                ...
                if (strlen($text) > 20000) {
                    ...
                    try {
                        ...
                        $insertThread = $con->prepare('
                            INSERT INTO `forumthreads`
                            VALUES(DEFAULT, :title, :text, :authorid, now(), DEFAULT, DEFAULT, :cat, DEFAULT, DEFAULT, 0)
                        ');
    ...
    protected function addForm() {
        ...
        try {
            ...
            $selectCategories = $con->query('
                ...
                ORDER BY `forumcategories`.`name` ASC
            ');
    ...
    public function delete($fromadmin = null) {
        ...
        try {
            ...
            $deletePosts = $con->prepare('
                DELETE FROM `forumposts`
                WHERE `forumposts`.`threadid` = :threadid
            ');
            $deletePosts->bindValue('threadid', $this->id, PDO::PARAM_INT);
            $deletePosts->execute();

            $deleteThread = $con->prepare('
                DELETE FROM `forumthreads`
                WHERE `forumthreads`.`id` = :threadid
            ');
            $deleteThread->bindValue('threadid', $this->id, PDO::PARAM_INT);
    ...
    public function edit() {
        ...
        if ((($this->author->getId() != $user->getId()) || $this->isClosed()) && !$user->isAdmin()) {
            ...
            if (isset($_POST['edit']) && (isset($_POST['cat']) && validField($_POST['cat'])) && (isset($_POST['title']) && validField($_POST['title'])) && (isset($_POST['text']) && validField($_POST['text']))) {
            ...
            if (strlen($title) > 37) {
                ...
                if (strlen($text) > 20000) {
                    ...
                    try {
                        ...
                        $updateThread = $con->prepare('
                            UPDATE `forumthreads`
                            SET `forumthreads`.`forumcategory` = :cat,
                                `forumthreads`.`title` = :title,
                                `forumthreads`.`text` = :text,
                                `forumthreads`.`editdate` = now()
                            WHERE `forumthreads`.`id` = :threadid
                        ');
                        ...
                        $updateThread->bindValue('threadid', $this->id, PDO::PARAM_INT);
        ...
        if ($user->isAdmin()) {
            ...
            if (isset($_POST['closed']) && $_POST['closed'] == 'on' && !$this->isClosed()) {
                ...
                try {
                    ...
                    $closeThread = $con->prepare('
                        UPDATE `forumthreads`
                        SET `forumthreads`.`closed` = 1
                        WHERE `forumthreads`.`id` = :id
                    ');
                ...
                try {
                    ...
                    $openThread = $con->prepare('
                        UPDATE `forumthreads`
                        SET `forumthreads`.`closed` = 0
                        WHERE `forumthreads`.`id` = :id
                    ');
    ...
    protected function editForm() {
        ...
        echo $twig->render(
            ...
            array(
                ...
                'currentcategory' => $this->forumcategory->getId(),
                'ispost' => false,
                'userisadmin' => $user->isAdmin(),
                'thread' => $this->returnArray()
            )
        );
